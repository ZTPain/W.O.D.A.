...
void SaveManager::LoadReplayActions(
  ...
  const std::function<void(ReplayActionEntry&)> reader = [&](ReplayActionEntry& entry) {
    ...
    ReadBytes(data, offset, length, entry.commandData);
  };

  ReadBytes(data, offset, length, outReplayActions, reader);
}
